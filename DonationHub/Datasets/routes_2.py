...
def index():
    ...
    headings = {"County Name", "Unemployed People", "Unemployed Rate" }
    ## join of unemployment, counties, and states
    ...
    # SELECT 
    result = db.session.query( states.ABBREVIATION, counties.COUNTYNAME, unemployment.UNEMPLOYED_PEOPLE, unemployment.UNEMPLOYMENT_RATE).filter(counties.COUNTYID==unemployment.COUNTYID, counties.STATEID==states.STATEID).all()
    print(result)
    result_join = getOnlyUnemploymentTable(['Maryland'],['Alaska'])
    ...
    if request.method == 'POST':
        ...
        return '<h1>State : {}, county: {}</h1>'.format(state.STATENAME, county.COUNTYNAME,result=result, headings=headings, result_join=result_join)
    return render_template('unemployment_table.html', form=form, state_list=state_list, county_list=county_list, result=result, headings=headings, result_join=result_join)
